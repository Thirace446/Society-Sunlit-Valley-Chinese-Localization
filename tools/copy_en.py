...
def mirror_en_us_with_log():
    ...
    logging.info(f"开始任务: 从 {src_root} 镜像 en_us.json 到 {dst_root}")
    
    count = 0
    
    # 2. 遍历查找
    # 查找所有子目录下的 en_us.json
    for src_file in src_root.rglob("en_us.json"):
        try:
            # 3. 计算相对路径
